...
namespace flair {
   namespace events {
      ...
      class IEventDispatcher
      {
         // Methods
      public:
         virtual void addEventListener(std::string type, std::function<void(std::shared_ptr<Event>)> listener, bool useCapture = false, int priority = 0, bool useWeakReference = false) = 0;
